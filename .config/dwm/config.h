...
#define FMANAGER "alacritty -e vifm"

// ---------------------------------- Fonts ------------------------------------

static const char *fonts[] = { "UbuntuMono Nerd Font:size=9:weight=bold:antialias=true:autohint:true" };
...
static Key keys[] = {
    ...
    // dmenu
    { 0,     XK_F3, spawn, SHCMD("dmenu_run") },

    // Window nav (rofi)
    { MODKEY, XK_d, spawn, SHCMD("rofi -show drun -disable-history") },

    // Terminal
    { MODKEY, XK_Return, spawn, SHCMD(TERMINAL) },

    // terminal
    { ALTMOD, XK_Return, spawn, SHCMD("tilix") },

    // File explorer
    { MODKEY,             XK_f, spawn, SHCMD("pcmanfm") },
    { MODKEY|ALTMOD, XK_f, spawn, SHCMD(FMANAGER) },

    // script de apagado
    { MODKEY|ALTMOD, XK_x, spawn, SHCMD("$HOME/.local/bin/power.sh") },

    // vscode
    { MODKEY, XK_s, spawn, SHCMD("code") },

    // wallpaper
    { MODKEY, XK_m, spawn, SHCMD("nitrogen") },
    ...
    // Volume F
    {MODKEY, XK_F2, spawn, SHCMD("pactl set-sink-volume @DEFAULT_SINK@ -2%")},
    {MODKEY, XK_F3, spawn, SHCMD("pactl set-sink-volume @DEFAULT_SINK@ +2%")},
    {MODKEY, XK_F4, spawn, SHCMD("pactl set-sink-mute @DEFAULT_SINK@ toggle")},

    // multimedia
    {MODKEY, XK_F5, spawn, SHCMD("playerctl stop")},
    {MODKEY, XK_F6, spawn, SHCMD("playerctl previous")},
    {MODKEY, XK_F6, spawn, SHCMD("playerctl play-pause")},
    {MODKEY, XK_F6, spawn, SHCMD("playerctl next")},

    // lock
    {MODKEY, XK_F11, spawn, SHCMD("i3lock -u -t -i /home/tax06d/Lock/1492163.png")},

    // Brillo
    {MODKEY, XK_F9, spawn, SHCMD("brightnessctl set 5%-")},
    {MODKEY, XK_F10, spawn, SHCMD("brightnessctl set +5%")},
